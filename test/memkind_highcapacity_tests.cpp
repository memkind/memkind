       ...
       test_TC_HiCapacityPreferred_TwoOrMoreNodes)
{
    ...
    if (high_capacity_nodes.size() < 2){
        GTEST_SKIP() << "This test requires minimum 2 highest capacity NUMA Nodes in the OS ";
    }
       ...
       test_TC_HiCapacityPreferred_alloc_until_full_numa)
{
    ...
    if (high_capacity_nodes.size() != 1){
        GTEST_SKIP() << "This test requires only 1 highest capacity NUMA Node in the OS ";
    }
    ProcStat stat;
    void *ptr;
    ...
    while (0.99 * sum_of_free_space > alloc_size * allocations.size()) {
        ...
        ASSERT_TRUE(high_capacity_nodes.count(numa_id));
    }
