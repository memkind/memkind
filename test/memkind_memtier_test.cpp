...
protected:
    ...
    void SetUp()
    {
        ...
        int res = memtier_builder_add_tier(builder, m_tier_default, m_tier_default_ratio);
        ASSERT_EQ(0, res);
        res = memtier_builder_add_tier(builder, m_tier_regular, m_tier_regular_ratio);
        ASSERT_EQ(0, res);
...
TEST_F(MemkindMemtierKindTest, test_ratio_malloc_free)
{
    ...
    for (i = 0; i < num_allocs; ++i) {
        ...
        // if actual ratio between default and regular is lower than desired,
        // new allocation should go to regular tier
        if ((op == 1) && (tier_regular_actual_ratio < m_tier_regular_ratio)) {
            ASSERT_GE(memtier_tier_allocated_size(m_tier_regular),
                      regular_alloc_size);
        }
    }
