...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "default") == 0) {
        ...
        memkind_stats_print(NULL, NULL, MEMKIND_STAT_PRINT_NONE);
    } else if (strcmp(argv[1], "stdout") == 0) {
        fd = 1; // write to stdout
        memkind_stats_print(&write_cb, (void *)&fd, MEMKIND_STAT_PRINT_NONE);
    } else if (strcmp(argv[1], "no_write_cb") == 0) {
        // default write_cb function should be called, write to stderr
        fd = 1; // write to stdout
        memkind_stats_print(NULL, (void *)&fd, MEMKIND_STAT_PRINT_NONE);
