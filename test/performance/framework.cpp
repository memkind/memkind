...
void Barrier::wait()
{
    ...
    m_waiting--;
    if (m_waiting == 0) {
        // Called by the last expected thread - notify all waiting threads and
        // exit
        m_cVar.notify_all();
...
// Worker class
Worker::Worker(uint32_t actionsCount, const vector<size_t> &allocationSizes,
    ...
    for (uint32_t i = 0; i < m_actionsCount; i++) {
        ...
        for (Operation *operation : testOperations) { // each operation
            if (operation->checkCondition(bucketSize)) {
                size_t size =
                    m_allocationSizes[m_allocationSizes.size() > 1
                                          ? rand() % m_allocationSizes.size()
                                          : 0];
                m_actions[i] = new Action(
                    operation, freeOperation, m_kind, size, log2(rand() % size),
                    sizeof(void *) *
                        (1 << ((rand() % Operation::MemalignMaxMultiplier))));
                break;
            }
        }
    }
...
PerformanceTest::PerformanceTest(size_t repeatsCount, size_t threadsCount,
...
void PerformanceTest::setOperations(
    const vector<vector<Operation *>> &testOperations, Operation *freeOperation)
{
...
void PerformanceTest::prepareWorkers()
{
    for (size_t threadId = 0; threadId < m_threadsCount; threadId++) {
        m_workers.push_back(new Worker(
            m_operationsCount, m_allocationSizes, m_freeOperation,
        ...
        if (m_executionMode == ExecutionMode::SingleInteration) {
            // In ManyIterations mode, operations will be set for each thread at
            // the beginning of each iteration
            m_workers.back()->init(
                m_testOperations[threadId % m_testOperations.size()],
                m_freeOperation);
        }
    }
}
...
Metrics PerformanceTest::getMetrics()
{
    ...
    metrics.executedOperations =
        m_durations.size() * m_threadsCount * m_operationsCount;
    metrics.totalDuration = totalDuration;
    metrics.repeatDuration =
        (double)totalDuration / ((uint64_t)m_durations.size() * NanoSecInSec);
    ...
    metrics.operationsPerSecond =
        (double)metrics.executedOperations * NanoSecInSec / totalDuration;
    metrics.avgOperationDuration =
        (double)totalDuration / metrics.executedOperations;
                                   ...
                                   const string &fileName)
{
    ...
    if (!fileName.empty()) {
        ...
        if ((f = fopen(fileName.c_str(), "a+"))) {
            fprintf(f, "%s;%s;%zu;%zu;%lu;%f;%f;%f;%f\n", suiteName.c_str(),
                    caseName.c_str(), m_repeatsCount, m_threadsCount,
                    metrics.executedOperations, metrics.operationsPerSecond,
                    metrics.avgOperationDuration, metrics.iterationDuration,
                    metrics.repeatDuration);
            fclose(f);
        }
    }
    printf("Operations/sec:\t\t\t%'f\n"
           ...
           metrics.operationsPerSecond, metrics.avgOperationDuration,
           metrics.iterationDuration, metrics.repeatDuration);
}

int PerformanceTest::run()
{
    if (m_testOperations.empty() || m_allocationSizes.empty() ||
    ...
    for (size_t repeat = 0; repeat < m_repeatsCount; repeat++) {
        ...
        if (m_executionMode == ExecutionMode::SingleInteration) {
            ...
            // Perform each operations list in separate iteration, for each
            // thread
...
{
    printf(
        "Test parameters: %lu repeats, %lu threads, %d operations per thread\n",
        m_repeatsCount, m_threadsCount, m_operationsCount);
