...
class Test_malloc_stats_print(object):
    ...
    def test_TC_MEMKIND_malloc_stats_print_all_opts_test(self):
        ...
        assert "Merged arenas stats:" not in output, \
               self.error_msg.format("MEMKIND_STAT_PRINT_OMIT_MERGED_ARENA")
        assert "Destroyed arenas stats:" not in output, \
               self.error_msg.format("MEMKIND_STAT_PRINT_OMIT_DESTROYED_MERGED_ARENA")
        assert "arenas[" not in output, \
               self.error_msg.format("MEMKIND_STAT_PRINT_OMIT_PER_ARENA")
        assert "bins:" not in output, \
               self.error_msg.format("MEMKIND_STAT_PRINT_OMIT_PER_SIZE_CLASS_BINS")
        assert "lextents" not in output, \
               self.error_msg.format("MEMKIND_STAT_PRINT_OMIT_PER_SIZE_CLASS_LARGE")
        assert "n_lock_ops" not in output, \
