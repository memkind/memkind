...
class Test_tiering_config_env(Helper):
    ...
    def test_negative_unsupported_policy(self):
        output = self.get_ld_preload_cmd_output(
            "MEMKIND_MEM_TIERING_CONFIG=DRAM:1,ABC", validate_retcode=False)

        assert output[0] == self.log_error_prefix + \
            "Unknown policy: ABC", "Wrong message"
    ...
    def test_negative_multiple_policies(self):
        ...
        # NOTE: only last query is threated as a policy, so first from two
        # polices will be threated as kind
        assert output[0] == self.log_error_prefix + \
