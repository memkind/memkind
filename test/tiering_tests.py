...
class Helper(object):
    ...
    default_policy = "POLICY:POLICY_STATIC_THRESHOLD"
...
class Test_tiering_config_env(Helper):
    ...
    @pytest.mark.parametrize("fsdax_cfg",
                             ["KIND:FS_DAX,PATH:/tmp/,MAX_SIZE:10G,RATIO:1",
                              "PATH:/tmp/,KIND:FS_DAX,MAX_SIZE:10G,RATIO:1",
                              "KIND:FS_DAX,MAX_SIZE:10G,PATH:/tmp/,RATIO:1",
                              "RATIO:1,MAX_SIZE:10G,KIND:FS_DAX,PATH:/tmp/"])
    ...
    @pytest.mark.parametrize("policy", ["POLICY:POLICY_STATIC_THRESHOLD"])
    ...
    def test_FSDAX(self, pmem_size):
        self.get_ld_preload_cmd_output(
            "KIND:FS_DAX,PATH:/tmp/,MAX_SIZE:" + pmem_size + ",RATIO:1;" +
    ...
    def test_FSDAX_check_only_fs_dax(self):
        ...
        self.get_ld_preload_cmd_output(
            "KIND:FS_DAX,PATH:" + pmem_path + ",MAX_SIZE:1G,RATIO:1;" +
            self.default_policy, log_level="2")
    ...
    def test_FSDAX_wrong_ratio(self):
        self.get_ld_preload_cmd_output(
            "KIND:FS_DAX,PATH:/tmp/,MAX_SIZE:10G,RATIO:a;" +
            self.default_policy,
            negative_test=True)

    def test_FSDAX_no_path(self):
        self.get_ld_preload_cmd_output(
            "KIND:FS_DAX,MAX_SIZE:10G,RATIO:1;" +
            self.default_policy,
            negative_test=True)
    ...
    @pytest.mark.parametrize("tier_str",
                             ["KIND:DRAM,RATIO:1",
                              "KIND:FS_DAX,PATH:/tmp/,MAX_SIZE:100M,RATIO:1",
                              "KIND:DRAM,RATIO:1;KIND:FS_DAX,PATH:/tmp/,"
                              "MAX_SIZE:100M,RATIO:1"])
    def test_negative_no_policy(self, tier_str):
        self.get_ld_preload_cmd_output(
            tier_str, negative_test=True)

    @pytest.mark.parametrize("policy",
                             ["POLICY:POLICY_STATIC_THRESHOLD",
                              "POLICY:POLICY_DYNAMIC_THRESHOLD"])
    def test_multiple_tiers(self, policy):
        ...
        self.get_ld_preload_cmd_output(
            "KIND:DRAM,RATIO:1;KIND:FS_DAX,PATH:/tmp/,MAX_SIZE:100M,RATIO:4;"
            "KIND:FS_DAX,PATH:/mnt,MAX_SIZE:150M,RATIO:8;" + policy,
            log_level="2")

    @pytest.mark.parametrize("wrong_tier",
                             ["KIND:non_existent,PATH:/mnt,MAX_SIZE:150M,"
                              "RATIO:8",
                              "KIND:FS_DAX,PATH:/non_existent,MAX_SIZE:150M,"
                              "RATIO:8",
                              "KIND:FS_DAX,PATH:/mnt,MAX_SIZE:1M,RATIO:8",
                              "KIND:FS_DAX,PATH:/mnt,MAX_SIZE:150M,RATIO:0",
                              "KIND:DRAM,RATIO:1"])
