...
class Test_tiering_config_env(Helper):
    ...
    @pytest.mark.parametrize("mtt_limits", ["LOW,SOFT,HARD",
                                            "LOW=1,SOFT::1",
                                            "LOWx:SOFTT:HART",
                                            ":::",
                                            "HARD:2x"])
    def test_env_var_mtt_limits_default_values(self, mtt_limits):
        out = self.get_ld_preload_cmd_output(
            "KIND:DRAM,RATIO:1;"
            + "KIND:KMEM_DAX,RATIO:4;"
            + "POLICY:DATA_MOVEMENT",
            memkind_log=True,
            mtt_limits=mtt_limits,
            log_level="2")
        low_limit_in_bytes = 1 * (1 << 20)
        soft_limit_in_bytes = 16 * (1 << 20)
        hard_limit_in_bytes = 1 * (1 << 30)
        msg_low_limit = (
            f"{self.memkind_log_prefix} MTT ranking low_limit"
            f" = {low_limit_in_bytes}"
        )
