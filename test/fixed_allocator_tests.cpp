...
TEST_F(FixedAllocatorTests, test_TC_MEMKIND_AllocatorUsage_VectorOfString_Test)
{
    typedef std::basic_string<char, std::char_traits<char>,
        ...
        fixed_string;
    typedef libmemkind::fixed::allocator<fixed_string> fixed_alloc;

    fixed_alloc alc{*alloc_source};
    libmemkind::fixed::allocator<char> st_alc{alc};

    std::vector<fixed_string, std::scoped_allocator_adaptor<fixed_alloc>> vec{
        alc};
    fixed_string arg{"Very very loooong striiiing", st_alc};
       ...
       test_TC_MEMKIND_AllocatorScopedUsage_MapOfIntString_Test)
{
    typedef std::basic_string<char, std::char_traits<char>,
        ...
        fixed_string;
    typedef int key_t;
    typedef fixed_string value_t;
    typedef std::pair<const key_t, value_t> target_pair;
    typedef libmemkind::fixed::allocator<target_pair> fixed_alloc;
    typedef libmemkind::fixed::allocator<char> str_allocator_t;
    typedef std::map<key_t, value_t, std::less<key_t>,
                     std::scoped_allocator_adaptor<fixed_alloc>>
        map_t;

    fixed_alloc map_allocator(*alloc_source);
    ...
    map_t target_map{std::scoped_allocator_adaptor<fixed_alloc>(map_allocator)};
