...
static bool is_kind_preferred(memkind_t kind)
{
...
MEMKIND_EXPORT long long memkind_get_total_memory(struct memkind *kind)
{
    struct bitmask *mask = NULL;
    long long total_memory = 0;
    bool all_nodes_ptr_used = false;

    if (is_kind_preferred(kind)) {
        log_err("memkind_get_total_memory() failed. %s kind is not supported.",
                kind->name);
        return -1;
    }
    ...
    if (mask) {
        ...
        total_memory = pmem_priv->max_size;
    }
