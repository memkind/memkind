                                             ...
                                             uintptr_t address)
{
    ...
    // The algorithm requires the compare exchange to be
    // inside the loop
    // if any other thread overwrites queue->head (there is a race),
    // the function loads the new `head` value to node->next
    // and retries setting the new head
