...
static struct bitmask *get_dax_kmem_nodemask()
{
    ...
    struct bitmask *nodes_mask = numa_bitmask_alloc(nodes_num);
    if (!nodes_mask)
        return NULL;
    ...
    daxctl_region_foreach_safe(ctx, region, _region) {
        daxctl_dev_foreach_safe(region, dev, _dev) {
            ...
            if (mem) {
                numa_bitmask_setbit(nodes_mask,
    ...
    if (numa_bitmask_weight(nodes_mask) != 0) {
        return nodes_mask;
    }
