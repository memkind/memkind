...
#define POLICY_DYNAMIC_THRESHOLD_TRIGGER    10
#define POLICY_DYNAMIC_THRESHOLD_CHANGE_VAL 25
                                          ...
                                          size_t size)
{
    ...
    for (i = 0; i < memory->size - 1; ++i) {
        float actual_ratio = memtier_kind_allocated_size(cfg[i].kind) /
            memtier_kind_allocated_size(cfg[0].kind);
        float ratio_diff = (1.0 - actual_ratio / cfg[i].kind_ratio);
        float change = (ratio_diff > POLICY_DYNAMIC_THRESHOLD_TRIGGER) ? 1
            : (ratio_diff < -POLICY_DYNAMIC_THRESHOLD_TRIGGER)         ? -1
                                                                       : 0;
        if (change > 0) {
            float new_val = (1.0 + thres[i].val) * change *
                POLICY_DYNAMIC_THRESHOLD_CHANGE_VAL;
