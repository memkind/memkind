...
memtier_policy_static_threshold_get_tier(struct memtier_kind *tier_kind)
{
    ...
    if (cfg[0].tier->alloc_size == 0) {
        return cfg[0].tier;
    }

    int i;
    int worst_tier = 0;
    float worst_ratio = 1.0 / cfg[0].tier_ratio;
    for (i = 1; i < tier_kind->size; ++i) {
        float ratio_distance = (float)cfg[i].tier->alloc_size /
            cfg[0].tier->alloc_size / cfg[i].tier_ratio;
