...
static MTTAllocator *g_mtt_allocator = NULL;
...
builder_movement_create_memory(struct memtier_builder *builder)
{
    ...
    // initialize DAX_KMEM here to avoid potential problems later
    void *temp = memkind_malloc(MEMKIND_DAX_KMEM_ALL, 10);
    *(volatile int *)temp = 0;
    memkind_free(MEMKIND_DAX_KMEM_ALL, temp);
    ...
    if (builder->mtt_limits == NULL) {
        limits.lowLimit = 1ul * 1024ul * 1024ul;     // 1 MB;
        limits.softLimit = 16ul * 1024ul * 1024ul;   // 16 MB;
        limits.hardLimit = 1024ul * 1024ul * 1024ul; // 1 GB;
    } else {
        limits.lowLimit = builder->mtt_limits->lowLimit;
        limits.softLimit = builder->mtt_limits->softLimit;
        limits.hardLimit = builder->mtt_limits->hardLimit;
    }
...
{
    if (memory->policy == MEMTIER_POLICY_DATA_MOVEMENT) {
        return mtt_allocator_malloc(g_mtt_allocator, size);
    }
...
{
    if (memory->policy == MEMTIER_POLICY_DATA_MOVEMENT) {
        return mtt_allocator_realloc(g_mtt_allocator, ptr, size);
    }
...
MEMKIND_EXPORT size_t memtier_usable_size(void *ptr)
{
    ...
    // causes memory corruption if MEMTIER_POLICY_DATA_MOVEMENT
    // is constructed outside of LD_PRELOAD case
    if (g_mtt_allocator) {
...
MEMKIND_EXPORT void memtier_kind_free(memkind_t kind, void *ptr)
{
    ...
    if (!kind) {
        ...
        // is constructed outside of LD_PRELOAD case
        if (g_mtt_allocator) {
            mtt_allocator_free(g_mtt_allocator, ptr);
            return;
        }
