                                ...
                                __off_t __offset)
{
    ...
    assert(allocator->internals.limits.hardLimit &&
           "hard limit cannot be is 0!");
    ...
    size_t temp_dram_size = 0ul, ndram_size = 0ul;

    do {
        temp_dram_size = atomic_load(&allocator->internals.usedDram);
        ndram_size = temp_dram_size + __len;
        if (temp_dram_size > allocator->internals.limits.hardLimit) {
        ...
        &allocator->internals.usedDram, &temp_dram_size,
...
///     - allocator->internals.limits.hardLimit
static void *mtt_allocator_mmap_single_init(void *arg, void *__addr,
                                            size_t __len, int __prot,
                                            int __flags, int __fd,
                                            __off_t __offset)
{
    MTTAllocator *allocator = arg;
    // TODO assert that the same area is not mmapped twice !!!
                                         ...
                                         const MTTInternalsLimits *limits)
{
    ...
    mmap_callback.wrapped_mmap = mtt_allocator_mmap_single_init;
