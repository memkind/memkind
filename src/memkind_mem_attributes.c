    ...
    *nodes_mask = calloc(sizeof(struct bitmask *), num_cpus);
    ...
    while ((init_node = hwloc_get_next_obj_by_type(topology, HWLOC_OBJ_NUMANODE,
        ...
        switch(attr) {
            ...
            default:
                log_fatal("Type not supported yet");
                ret = MEMKIND_ERROR_UNAVAILABLE;
                goto error;
        }
        ...
        if (node_variant == NODE_VARIANT_SINGLE &&
            ...
            log_err("Multiple NUMA Nodes have the same value for memory attribute for init node %d.",
                    init_node->os_index);
            goto error;
        }

        // populate memory attibute nodemask to all CPU's from innitiator NUMA node
