...
#define use_mbind 1
    ...
    bitmask = numa_bitmask_alloc(max_node);
    struct bitmask nodemask_dax_kmem = {NUMA_NUM_NODES, nodemask.n};
    numa_bitmask_clearall(&nodemask_dax_kmem);

    // Page metadata address aligned to the operating system page
    const size_t page_size = sysconf(_SC_PAGESIZE);
    const size_t page_mask = ~(page_size - 1);
    os_page_addr = (void *)((uintptr_t)page_metadata_addr & page_mask);

    if (memory_type == DRAM) {
        nodemask = get_dram_nodemask();

        ret = mbind(os_page_addr, page_size, MPOL_BIND, nodemask.n,
