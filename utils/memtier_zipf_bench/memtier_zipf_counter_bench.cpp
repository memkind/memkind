...
class MultiVMAAllocator: public Allocator
{
    ...
    /// start of the whole mapped area
    void *areaStart;
    /// start of unused area
    void *unusedAreaStart;
    ...
    /// debug variable
    /// in this test, once everything is allocated and the accesses are started
    /// only freelists should be used
    bool temp_deleted_something = false;

public:
    MultiVMAAllocator(size_t vma_interval)
        : pageSize(sysconf(_SC_PAGESIZE)), vmaInterval(vma_interval)
    {
        areaStart = mmap(NULL, TOTAL_SIZE, PROT_NONE,
                         MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
        unusedAreaStart = areaStart;
    ...
    void *malloc(size_t size)
    {
        ...
        if (size_to_malloc <= pageSizeLeft) {
            ...
            ret = unusedAreaStart;
            unusedAreaStart = reinterpret_cast<void *>(
                reinterpret_cast<uint8_t *>(unusedAreaStart) + size_to_malloc);
        ...
        unusedAreaStart = reinterpret_cast<void *>(
            reinterpret_cast<uint8_t *>(unusedAreaStart) + to_add);
        pageSizeLeft = 0ul;
#elif defined(PACKED_VMA_ALLOCATOR)
        size_t nof_pages =
            ceil(static_cast<double>(size_to_malloc - pageSizeLeft) / pageSize);
        if (vmaCounter++ > vmaInterval) {
            nof_pages++;
            nof_vmas += 2ul;
            vmaCounter = 0ul;
        ...
        if (vmaCounter++ > vmaInterval) {
            ...
            // no-VMA allocator
            void *tmmap_start = reinterpret_cast<void *>(
                reinterpret_cast<uint8_t *>(unusedAreaStart) + pageSizeLeft);
            assert(reinterpret_cast<uint8_t *>(unusedAreaStart) + pageSizeLeft +
                       pageSize <=
                   reinterpret_cast<uint8_t *>(areaStart) + TOTAL_SIZE);
            vmaCounter = 0ul;
            // auxiliary mmap
            ret = mmap(reinterpret_cast<uint8_t *>(unusedAreaStart) +
                           pageSizeLeft,
        ...
        pageSizeLeft = 0ul;
#endif
        void *mmap_start = reinterpret_cast<void *>(
            reinterpret_cast<uint8_t *>(unusedAreaStart) + pageSizeLeft);
        size_t pages_size = nof_pages * pageSize;
        pageSizeLeft += pages_size - size_to_malloc;
        assert(reinterpret_cast<uint8_t *>(unusedAreaStart) + pages_size <=
               reinterpret_cast<uint8_t *>(areaStart) + TOTAL_SIZE);
        ...
        *reinterpret_cast<size_t *>(unusedAreaStart) = size;
        ret = reinterpret_cast<uint8_t *>(unusedAreaStart) + sizeof(size_t);
        unusedAreaStart = reinterpret_cast<void *>(
            reinterpret_cast<uint8_t *>(unusedAreaStart) + size_to_malloc);
        return ret;
    }
    ...
    void free(void *ptr)
    {
        void *size_ptr = reinterpret_cast<void *>(
            reinterpret_cast<uint8_t *>(ptr) - sizeof(size_t));
                       ...
                       size_t internal_repetitions)
{
    ...
    for (size_t internal_repetition = 0ul;
         internal_repetition < internal_repetitions; ++internal_repetition) {
...
create_loader_creators(TestCase test_case, size_t nof_allocations,
                       ...
                       const MTTInternalsLimits *limits, size_t vma_interval)
{
    const double s = 2.0;
        ...
        case GeneralPolicy::GeneralType::RawAllocator:
        {
            switch (policy.allocator) {
                ...
                case RawAllocator::MULTI_VMA:
                    allocator =
                        std::make_shared<MultiVMAAllocator>(vma_interval);
                    break;
    ...
    for (size_t type_idx = 0, k = 1; type_idx < types_count; ++type_idx, ++k) {
...
static int parse_opt(int key, char *arg, struct argp_state *state)
{
    ...
    switch (key) {
        ...
        case KEY_INTERNAL_ITERATIONS:
        {
            assert(arg && "argument for internal repetitions not supplied");
...
static struct argp_option options[] = {
    ...
    {"no_malloc_iterations", KEY_INTERNAL_ITERATIONS, "size_t", 0,
     "Number of iterations before freeing memory", 5},
    {0}};
