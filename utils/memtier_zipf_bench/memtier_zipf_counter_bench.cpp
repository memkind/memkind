...
public:
    ...
    void *malloc(size_t size)
    {
        ...
        if (size_to_malloc <= pageSizeLeft) {
            ...
            ret = areaCstart;
            areaCstart = reinterpret_cast<void *>(
                reinterpret_cast<uint8_t *>(areaCstart) + size_to_malloc);
        ...
        areaCstart = reinterpret_cast<void *>(
            reinterpret_cast<uint8_t *>(areaCstart) + to_add);
        pageSizeLeft = 0ul;
        ...
        if (vmaCounter++ > vmaInterval) {
            ...
            void *tmmap_start = reinterpret_cast<void *>(
                reinterpret_cast<uint8_t *>(areaCstart) + pageSizeLeft);
            assert(reinterpret_cast<uint8_t *>(areaCstart) + pageSizeLeft +
                       pageSize <=
                   reinterpret_cast<uint8_t *>(areaStart) + TOTAL_SIZE);
            vmaCounter = 0ul;
            // auxiliary mmap
            ret = mmap(reinterpret_cast<uint8_t *>(areaCstart) + pageSizeLeft,
