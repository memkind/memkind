...
class Loader
{
    ...
    uint64_t GenerateAccessOnce()
    {
        ...
        switch (test_case) {
            ...
            case (TC_RANDOM_COPY):
                for (size_t it = 0; it < iterations; ++it) {
                    for (size_t i = 0; i < data_size_u64 - CACHE_LINE_SIZE_U64;
                         i += CACHE_LINE_SIZE_U64) {
                        std::uniform_int_distribution<uint64_t> distr(
                            0, data_size_u64);
                        size_t index = distr(generator);
                        // RFC: Make this malloc() twice as large than for
                        // read-modify-write?
                        static_cast<volatile uint64_t *>(data.get())[index] =
                            static_cast<volatile uint64_t *>(
                                data.get())[index + CACHE_LINE_SIZE_U64];
                    }
                }
                break;
            case (TC_SEQ_COPY):
                for (size_t it = 0; it < iterations; ++it) {
                    for (size_t i = 0; i < data_size_u64 - CACHE_LINE_SIZE_U64;
                        ...
                        static_cast<volatile uint64_t *>(data.get())[i] =
                            static_cast<volatile uint64_t *>(
                                data.get())[i + CACHE_LINE_SIZE_U64];
                    }
                }
                break;
        }
...
public:
    Loader(tc_test_case_t test_case, std::shared_ptr<void> data,
                                                     ...
                                                     size_t pmem_dram_ratio)
{
    ...
    memtier_builder_add_tier(m_tier_builder, MEMKIND_DAX_KMEM, pmem_dram_ratio);
...
public:
    LoaderCreator(tc_test_case_t test_case, size_t size, size_t iterations,
                  std::shared_ptr<memtier_memory> memory)
        : test_case(test_case),
          size(size),
          iterations(iterations),
...
create_loader_creators(tc_test_case_t test_case, size_t min_size,
                       size_t types_count, size_t total_iterations,
                       memtier_policy_t policy, size_t pmem_dram_ratio)
{
    const double s = 2.0;
...
BenchResults run_test(const RunInfo &info)
{
    ...
    std::vector<LoaderCreator> loader_creators = create_loader_creators(
        ...
        info.policy, info.pmem_dram_ratio);
...
static int parse_opt(int key, char *arg, struct argp_state *state)
{
    ...
    switch (key) {
        ...
        case 'm':
            ...
            break;
        case 'A':
            args->test_case = TC_RANDOM_INC;
            break;
        case 'B':
            args->test_case = TC_SEQ_INC;
            break;
        case 'C':
            args->test_case = TC_RANDOM_COPY;
            break;
        case 'D':
            args->test_case = TC_SEQ_COPY;
            break;
        case 'i':
            assert((size_t)arg > 0 &&
...
int main(int argc, char *argv[])
{
    ...
    RunInfo info = {
        ...
        .policy = args.policy,
        .pmem_dram_ratio = PMEM_DRAM_RATIO,
    };
