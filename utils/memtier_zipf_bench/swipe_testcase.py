...
def calculate_dram_limits(proportions: (int, int)) -> (int, int):
    if len(proportions) != 2:
        raise ValueError(
            'Invalid ratio format, expected: "[0-9]*:[0-9]*", e.g. "1:8"'
        )
    dram_soft_limit = int(proportions[0] / (proportions[0] + proportions[1]))
    # align limit
    dram_soft_limit = (
...
def run(
    ...
    dram_low_limit, dram_soft_limit = calculate_dram_limits(proportions)
