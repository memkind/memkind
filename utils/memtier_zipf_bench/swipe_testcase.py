...
def parse_execution_times(output):
    values = re.findall(
        "Measured execution time \[malloc \| access\]: \[([0-9]*)\|([0-9]*)\]", output
    )[0]
    values


def check_mem_usage_by_node(dram_nodes, pmem_nodes):
    result = subprocess.run(["numastat", "-m"], stdout=subprocess.PIPE)
    out = result.stdout.decode()
    usage_data = re.findall("MemUsed(.*?)\n", out)[
        0
    ]  # TODO this takes into account cached/buffered memory as well ...
    usage_data = [float(data) for data in usage_data.split(" ") if data != ""][:-1]
    dram = 0.0
    pmem = 0.0
    used_nodes = set()
...
def check_mem_usage():
    DRAM_NODES = [0, 1]
    PMEM_NODES = [2, 3]
    return check_mem_usage_by_node(DRAM_NODES, PMEM_NODES)
...
def run(
    ...
    dram_before, pmem_before = check_mem_usage()
    proportions = [int(val) for val in ratio.split(":")]
    if len(proportions) != 2:
        raise ValueError("invalid ratio format")
    dram_soft_limit = str(int(proportions[0] / (proportions[0] + proportions[1])))
    ...
    while process.poll() is None:
        mem_usages.append(check_mem_usage())
    ...
    dram_after, pmem_after = check_mem_usage()
    avg_dram_use -= (dram_before + dram_after) / 2
    avg_pmem_use -= (pmem_before + pmem_after) / 2
...
# how does it work: for each possible combination of memory_used, ratios and element_sizes, run test and output results
