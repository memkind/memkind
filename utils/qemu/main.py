...
class GuestConnection():
    ...
    def _run_memkind_test(self, c: fabric.Connection) -> None:
        ...
        Run memkind test
        """
        # TODO check if user don't need to rerun install
    ...
    def _shutdown(self, c: fabric.Connection, warn=False) -> None:
        ...
        Shutdown guest
        """
        c.run('sudo shutdown now', echo=True, warn=warn)
...
def parse_topology_xml(tpg_file_name: str) -> TopologyCfg:
    ...
    try:
        ...
        tpg_cfg['hmat'] = 'hmat=on' in libvirt_args
        tpg_cfg['cpu_options'] = re.search('(?=-smp)(.*)threads=[0-9]+', libvirt_args).group(0)
