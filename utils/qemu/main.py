...
class GuestConnection:
    ...
    def _build_and_reinstall_memkind(self, c: fabric.Connection) -> None:
        ...
        """
        result = c.run('nproc', **self._echo_option)
        nproc = int(result.stdout.strip())
        c.run('make distclean', **self._echo_option, warn=True)
        c.run('./autogen.sh', **self._echo_option)
        c.run(f'./configure {self._memkind_configure_params}', **self._echo_option)
        c.run(f'make -j {nproc}', **self._echo_option)
        c.run(f'make checkprogs -j {nproc}', **self._echo_option)
        c.run('sudo make uninstall', **self._echo_option)
        c.run('sudo make install', **self._echo_option)
        c.run('sudo ldconfig', **self._echo_option)
    ...
    def _run_codecov_script(self, c: fabric.Connection) -> None:
        ...
        c.run(f'{codecov_script} {MEMKIND_GUEST_PATH}', **self._echo_option)
