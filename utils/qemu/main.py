...
class GuestConnection():
    ...
    def _shutdown(self, c: fabric.Connection) -> None:
        ...
        status = None
        try:
            # TODO check why shutdown return status 1
            status = c.run('sudo shutdown now', echo=True, warn=True)
        except Exception as e:
            print(f"something RESULT {status.exited} stdout {status.stdout} went wrong with shutdown: {e}")
