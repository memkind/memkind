...
# TODO hanlding scaling the memory in xml
...
class GuestConnection():
    ...
    @_logger
    def _shutdown(self, c: fabric.Connection, warn=False) -> None:
        """
        Shutdown guest
        """
        #TODO check why in run all the test status is not equal 0
        status = c.run('sudo shutdown now', echo=True, warn=warn)
        ...
        with fabric.Connection(**self._connection_params, inline_ssh_env=True) as c:
            try:
                with c.cd(MEMKIND_GUEST_PATH):
                    ...
                    if force_reinstall:
                        self._build_and_reinstall_memkind(c)
                print(f'QEMU development is ready to use via SSH or VNC ({VNC_DEFAULT_PORT + VNC_PORT})')
...
class QEMU():
    ...
    def _connect_option(self) -> str:
        ...
        - tcp on port 10022
        """
        return f'-vnc :{VNC_PORT} -netdev user,id=net0,hostfwd=tcp::{TCP_PORT}-:22 -device virtio-net,netdev=net0'
