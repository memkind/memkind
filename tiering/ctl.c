...
static int ctl_parse_pmem_size(const char *pmem_size_str, size_t *pmem_size)
{
    ...
    int ret = regcomp(&regex, "^[[:digit:]]+[kMG]{0,1}$", REG_EXTENDED);
    if (ret) {
        log_err(
