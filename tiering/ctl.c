...
{
    char env_var_copy[MAX_ENV_STRING] = {0};
    strncpy(env_var_copy, env_var_string, MAX_ENV_STRING - 1);
    ...
    qbuf = strtok_r(env_var_copy, CTL_STRING_QUERY_SEPARATOR, &sptr);
    if (qbuf == NULL) {
        log_err("No valid query found in: %s", env_var_copy);
        return NULL;
    }
    ...
    if (thresholds_var_string) {
        char thresholds_var_copy[MAX_ENV_STRING] = {0};
        strncpy(thresholds_var_copy, thresholds_var_string, MAX_ENV_STRING - 1);
        ...
        qbuf = strtok_r(thresholds_var_copy, CTL_STRING_QUERY_SEPARATOR, &sptr);
        if (qbuf == NULL) {
            log_err("No valid thresholds found in: %s", thresholds_var_copy);
            goto destroy_builder;
        }
